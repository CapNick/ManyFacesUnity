		...
		void Update () {
		    ...
		    foreach (GameObject face in Faces) {
				if (faceIndex < 18) {
					...
					face.GetComponent<Face2> ().UpdateLookingPosition (lastFacePos[0]);
					faceIndex++;
				} else if (faceIndex < 36) {
					if (numFaces == 2) {
						face.transform.LookAt (lastFacePos [1]);
						face.GetComponent<Face2> ().UpdateLookingPosition (lastFacePos [1]);
						faceIndex++;
					} else {
						face.transform.LookAt (lastFacePos[0]);
						face.GetComponent<Face2> ().UpdateLookingPosition (lastFacePos[0]);
					...
					if (numFaces == 3) {
						...
						face.GetComponent<Face2> ().UpdateLookingPosition (lastFacePos [2]);
						faceIndex++;
					} else {
						face.transform.LookAt (lastFacePos[0]);
						face.GetComponent<Face2> ().UpdateLookingPosition (lastFacePos[0]);
