...
//				        faceGameObject.transform.position = new Vector3(x + x*FaceWidth+FaceWidth,-(y + y*FaceHeight+FaceHeight));
				        face.transform.position = new Vector3((-FacesPerLine+x)+ScreenWidth,0);
        ...
        public void OnDrawGizmosSelected() {
            ...
            for (int y = 0; y < FacesLines; y++) {
                for (int x = 0; x < FacesPerLine; x++) {
                    Gizmos.DrawCube(new Vector3((-ScreenWidth+FaceWidth)/2 + (FaceWidth) * x, 
                        (-ScreenHeight+FaceHeight)/2  + (FaceHeight)* y + 0.537174f), new Vector3(FaceWidth,FaceHeight,1f));
                }
            }
        }
    }
